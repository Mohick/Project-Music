...
function  SearchResult() {
    ...
    useEffect(()=>{
        axios.get('http://localhost:3000/discover/format-json/').then((response)=>{
           ...
           const items = data.filter((data) => data.titleMusical.includes(title))
           setitem(items)
        })
    },[])
   return <div className="container mt-4 py-3 m-auto">
      <div className='text-white  text-2xl  mb-2 font-bold'>Result Search <span className='text-red-600'>&#35;</span></div>
            <div className="grid  gap-2 border rounded border-white p-1  sm:grid-cols-3  lg:grid-cols-4 xl:grid-cols-5  search__sort--items pb-3 max-h-96  overflow-y-auto overflow-x-hidden">
                {item.map((item,index) =>{
