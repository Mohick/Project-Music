...
class HandleUpdateAccount {
    ...
    handleSubmit(account) {
       ...
       axios.get(`http://localhost:3000/account/views-data/edit/checkBeformUpdate?id=${account._id}&email=${inputEmail}`)
       .then (res =>{
           if(JSON.parse(res.data.email)) {
            ...
            const inputPassword = document.querySelector(".setting__box__edit--input--password").querySelector("input").value.trim()
                ...
                axios.patch(`http://localhost:3000/account/views-data/update/client`,obAccount,{
